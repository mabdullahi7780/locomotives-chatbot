...
export class FieldGuard {
  ...
  private validateReadTheseFields(
    ...
    for (const field of fields) {
      ...
      // 3. Check if field matches known safe patterns from adapter
      if (this.isKnownSafeAdapterPattern(trimmedField)) {
        continue; // Allow this field
      }

      // 4. Check allowlist from catalog
  ...
  /**
   * Check if field matches known safe patterns generated by the adapter layer.
   * Sensitive fields are already blocked by isDenied().
   */
  private isKnownSafeAdapterPattern(fieldPath: string): boolean {
    const lowerPath = fieldPath.toLowerCase();

    // Root-level safe fields
    const safeRootFields = [
      "status", "$", "credit", "outofusedays", "assetid",
      "nextexpirydate", "title", "testcode", "locono", "name", "muid",
      "date", "outofuse", "noncompliant", "dailydue",
    ];

    if (safeRootFields.includes(lowerPath)) {
      return true;
    }

    // Safe prefix patterns
    const safePrefixes = [
      "value.summary",
      "value.assetdata",
      "status",
    ...
    // Allow fields with dynamic key placeholders like <assetId>
    if (/<[a-z_]+>/i.test(fieldPath)) {
      return true;
    }

    // Allow array notation patterns
    if (/^\[\]\./.test(fieldPath) || /^\[\d+\]\./.test(fieldPath)) {
      return true;
    }
  ...
  /**
   * Check if field matches known safe patterns (for adapter-generated fields)
   * This provides flexibility while still blocking sensitive data
   */
  private isKnownSafePattern(fieldPath: string): boolean {
    const lowerPath = fieldPath.toLowerCase();

    // Known safe prefixes from getDashBoardData
    const safePrefixes = [
      "status",
      "value.summary",
      "value.assetdata",
    ...
    // Check if field starts with a safe prefix
    for (const prefix of safePrefixes) {
      if (lowerPath.startsWith(prefix)) {
        return true;
      }
    ...
    // Allow fields with <assetId> placeholder pattern
    if (lowerPath.includes("<assetid>")) {
      return true;
    }

    // Allow array notation patterns
    if (/^\[\]\./.test(lowerPath) || /\[\d+\]/.test(lowerPath)) {
      return true;
    }

    // Allow common dashboard data patterns
    if (/^value\.assetdata\.[^.]+/.test(lowerPath)) {
      return true;
    }

    // Allow <assetId> keyed patterns
    if (/^<[^>]+>\./.test(lowerPath)) {
      return true;
    }
