...
export class FieldGuard {
   ...
   * FIX #1: NO broad wildcards - only catalog-defined patterns
   */
  private validateReadTheseFields(
    ...
    // Build allowlist ONLY from recommended calls (no broad wildcards!)
    ...
    for (const field of fields) {
      ...
      // 1. Check denylist FIRST (absolute block) - FIX #3: segment-based matching
      ...
      // 3. Check allowlist (must match at least one pattern from catalog)
