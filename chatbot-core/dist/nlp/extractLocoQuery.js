...
function looksLikeCountNumber(input, start, end) {
    ...
    if (/\b(show|list|display|get|give|find)\s*$/.test(left) &&
        /^\s*(inspections?|locos?|locomotives?|units?|engines?|records?|items?|results?)\b/.test(right))
        return true;
    // "number of 100 ...", "count of 50 ..."
    if (/\b(number\s+of|count\s+of)\s*$/.test(left))
        return true;
    return false;
}
 ...
 */
function hasDomainContext(input) {
